...
#define MOTHERBOARD 3  //AUGC DoodleBOT
...
#if MOTHERBOARD == 3
#define STEPPER_STEPS_PER_TURN    (128.0)
#else
...
#define SERVO_PIN1      (10)  // pin 10 is SS
#define SERVO_PIN2      (9)
#if MOTHERBOARD == 3
#define SERVO_PIN       (2)
#else
...
#define M2_PIN3 A2
#define M2_PIN4 A3
#define M1_STEP(s, d) d>0 ? m1.step(s) : m1.step(-s)
#define M2_STEP(s, d) d>0 ? m2.step(s) : m2.step(-s)
#define M1_ONESTEP(x)  x>0 ? m1.step(1) : m1.step(-1)
#define M2_ONESTEP(x)  x>0 ? m2.step(1) : m2.step(-1)
#endif
...
#endif
#if MOTHERBOARD == 3
Stepper m1(STEPS_PER_TURN, M1_PIN1, M1_PIN3, M1_PIN2, M1_PIN4);
Stepper m2(STEPS_PER_TURN, M2_PIN1, M2_PIN3, M2_PIN2, M2_PIN4);
#endif
...
//------------------------------------------------------------------------------
// Change pen state.
//@TODO: slow down the servo movement
void setPenAngle(int pen_angle) {
  int target=pen_angle;

  if(posz!=target) {
    if(target<PEN_DOWN_ANGLE) target=PEN_DOWN_ANGLE;
    if(target>PEN_UP_ANGLE  ) target=PEN_UP_ANGLE;

    for(int i = posz; i != target; target-posz > 0 ? i++ : i--) {
      s1.write( i );
      delay(PEN_DELAY);
    }
    posz=target;
...
void disable_motors() {
  ...
  digitalWrite(M1_PIN3, LOW); digitalWrite(M2_PIN4, LOW);
#endif
}
